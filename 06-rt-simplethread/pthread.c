...
void *counterThread(void *threadp) {
  ...
  printf(
      ...
      "core=%d\n",
      threadParams->threadIdx, thread_dt.tv_sec,
      (thread_dt.tv_nsec / NSEC_PER_MSEC),
      (thread_dt.tv_nsec / NSEC_PER_MICROSEC), thread_dt.tv_nsec,
      sched_getcpu());
...
int main(int argc, char *argv[]) {
  ...
  for (idx = 0; idx < NUM_THREADS; idx++) {
    ...
    rc = pthread_attr_setinheritsched(&rt_sched_attr[idx],
  ...
  for (idx = 0; idx < NUM_THREADS; idx++) pthread_join(threads[idx], NULL);
