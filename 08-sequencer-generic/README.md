...
modulo counting — the scaffold for the camera-pipeline final
project. Includes `clock_times.c` for probing Linux clock resolutions
and the `raspbian-ccr/` kernel module enabling user-space access to
the ARM cycle counter.

The vendored `sequencer_generic/` here omits the camera-specific files
(`capture.c`, `capturelib.c`, `seqv4l2.c`, `seqgen3.c`) and has
comment-only edits; top-level `clock_times.c` adds a
`CLOCK_MONOTONIC_COARSE` loop. `generic-sequencer.cpp` is the C++
rewrite (priority-queue release list).
...
seven-service camera pipeline is not represented.  It shows the
release-list idea, not yet the RM-priority behavior the C programs
demonstrate.
